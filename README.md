...
```text
outputs/figures/learning_curves/all_models_learning_curves_shared_axes.png
outputs/figures/learning_curves/all_models_validation_loss_overlay.png
outputs/figures/learning_curves/all_models_validation_macro_f1_overlay.png
outputs/figures/learning_curves/all_models_validation_accuracy_overlay.png
```
...
1차 실험 최종 결과는 `outputs/experiment1/metrics/all_experiments_full.json` 기준입니다. 표의 validation 성능은 best checkpoint 선택에 사용된 점수이고, test 성능은 선택된 checkpoint를 AI Hub Validation 기반 test split에서 평가한 결과입니다.
...
## Reorganized Experiment Structure

최종 보고서와 발표 자료 정리를 위해 데이터 가공, 1차 실험, 2차 실험 코드를 폴더 단위로 분리했습니다. 기존 루트 `scripts/*.py` 파일은 호환성을 위해 남겨두고, 최종 재현용 구조는 아래 폴더를 기준으로 사용합니다.

```text
scripts/data_processing/   # AI Hub 압축 해제, manifest 생성, frame 추출, split 생성
...
1차 실험과 2차 실험은 입력으로 사용하는 keypoint의 의미가 다릅니다. 1차 실험은 XML의 GT keypoint를 분류기에 직접 입력하여 pose 정보 자체가 얼마나 강력한지 확인한 실험이고, 2차 실험은 실제 추론 환경에 가깝게 RGB 이미지에서 keypoint를 먼저 예측한 뒤 downstream 분류를 수행하는 구조입니다.

Fusion 구조도에서는 RGB branch와 keypoint branch를 concat 이전까지 분리해서 표시했습니다. 즉 두 modality는 각각의 encoder와 temporal module을 지난 뒤에만 `Concat` 또는 `Attention` 단계에서 결합됩니다.

![Experiment 1 model architecture](docs/assets/model_architectures/experiment1_architecture.png)

![Experiment 2 model architecture](docs/assets/model_architectures/experiment2_architecture.png)

### Experiment1: GT Keypoint Baselines

1차 실험은 XML에 라벨링된 GT keypoint를 직접 입력으로 사용한 baseline 비교입니다.
...
최종 보고서에서 1차 실험 숫자가 바뀌지 않도록 기존 non-smoke 결과물을 `outputs/experiment1/`에 archive했습니다. 보고서용 수치는 이 폴더의 `README.md`와 `metrics/` 파일을 기준으로 사용합니다.
...
### Experiment2: Predicted Keypoint Downstream

2차 실험은 1차 실험에서 GT keypoint 기반 모델이 가장 높은 성능을 보인 이유가 라벨링된 동작 GT 정보를 직접 사용했기 때문이라는 가설에서 출발합니다. 따라서 XML keypoint를 GT로 사용해 `RGB image -> keypoint` 예측 모델을 먼저 학습하고, 그 예측 keypoint 기반 downstream 분류 성능을 비교합니다.
...
2. RGB-only classifier 학습/평가
3. predicted keypoint-only classifier 학습/평가
4. RGB + predicted keypoint fusion classifier 학습/평가
5. outputs/experiment2/ 아래 결과 저장
```
...
scripts/data_processing/          # organized data processing scripts
scripts/experiment1/              # organized first experiment runner
scripts/experiment2/              # organized second experiment runner
...
outputs/experiment1/              # first experiment outputs
outputs/experiment2/              # second experiment outputs
