...
class FusionAttentionModel(nn.Module):
    def __init__(
        ...
        self.video_rnn = nn.GRU(attn_dim, video_hidden_dim, batch_first=True)
        self.pose_rnn = nn.GRU(attn_dim, pose_hidden_dim, batch_first=True)
        self.classifier = nn.Sequential(
            nn.Dropout(0.4),
            nn.Linear(video_hidden_dim + pose_hidden_dim, 256),
            nn.ReLU(inplace=True),
    ...
    def forward(self, frames: torch.Tensor | None = None, keypoints: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.freeze_backbone:
            with torch.no_grad():
                video_features = self.encoder(x).reshape(bsz, steps, -1)
        else:
            video_features = self.encoder(x).reshape(bsz, steps, -1)
        video_tokens = self.video_proj(self.video_feature_norm(video_features))

        pose = self.pose_temporal(keypoints.transpose(1, 2)).transpose(1, 2)
        pose_tokens = self.pose_proj(pose)
        ...
        pose_tokens = self.attn_norm(pose_tokens + self.attn_dropout(attn_out))

        _, video_hidden = self.video_rnn(video_tokens)
        _, pose_hidden = self.pose_rnn(pose_tokens)
        fused = torch.cat([video_hidden[-1], pose_hidden[-1]], dim=-1)
        return self.classifier(fused)
